...
function Player:GetPointSum( nStyle, szUID )
	return (PlayerPoints[szUID] and PlayerPoints[szUID][nStyle]) and PlayerPoints[szUID][nStyle] or 0
end
...
function Player:SetRankMedal( ply, nPos )
	MySQL:Start( "SELECT t1.szUID, (SELECT COUNT(*) + 1 FROM game_times AS t2 WHERE szMap = '" .. game.GetMap() .. "' AND t2.nTime < t1.nTime AND nStyle = " .. ply.Style .. ") AS nRank FROM game_times AS t1 WHERE t1.szMap = '" .. game.GetMap() .. "' AND t1.nStyle = " .. ply.Style .. " ORDER BY nRank ASC LIMIT 3", function(Query)
...
function Player:LoadTop()
	...
	MySQL:Start( "SELECT szPlayer, SUM(nPoints) as nSum FROM game_times WHERE (nStyle = " .. _C.Style.Normal .. " OR nStyle = " .. _C.Style.Bonus .. ") GROUP BY szUID ORDER BY nSum DESC LIMIT " .. TopLimit, function(Normal)
	...
	MySQL:Start( "SELECT szPlayer, SUM(nPoints) as nSum FROM game_times WHERE (nStyle = " .. _C.Style.SW .. " OR nStyle = " .. _C.Style.HSW .. ") GROUP BY szUID ORDER BY nSum DESC LIMIT " .. TopLimit, function(Angled)
...
function Player:SendRemoteWRList( ply, szMap, nStyle, nPage, bUpdate )
	...
	if szMap == game.GetMap() then
		...
		return UI:SendToClient(ply, "wr", Timer:GetRecordList( nStyle, nPage ), nStyle, nPage, Timer:GetRecordCount( nStyle ))
	end
	...
	if bZero or SendCount == 0 then
		...
		if bUpdate then
			...
			UI:SendToClient(ply, "wr", SendData, nStyle,  nPage, SendCount)
		else
			UI:SendToClient(ply, "wr", SendData, nStyle, nPage, SendCount, szMap)
