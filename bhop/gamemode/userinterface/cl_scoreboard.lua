...
-- Text
local SCORE_TITLE = "Kawaii Clan - Bunny Hop"
local SCORE_PLAYERS = "%s/%s Players Online"
local SCORE_CREDITS = "Gamemode based off Flow Network by Gravious, edited by justa and Niflheimrx"
...
local function CreateScoreboard()
	...
	local function CreatePlayerRow(pl, isBot)
		...
		row.Paint = function(self, width, height)
			...
			draw.SimpleText(isBot and "WR Bot" or pRank[1], "hud.subtitle", 12, 20, isBot and text_colour or pRank[2], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			...
			if (rank != "User") then
				draw.SimpleText(string.upper(rank), "hud.subinfo2", (distance * 1.5) + namewidth + 2, 19 + nameheight/2, pl:GetObserverMode() ~= OBS_MODE_NONE and Color(150, 150, 150, 255) or text_colour2, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
			end
			...
			draw.SimpleText(latency, "hud.subtitle", width - 12, 20, text_colour, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
