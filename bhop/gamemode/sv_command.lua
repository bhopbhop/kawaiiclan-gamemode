...
function Command:Init()
	...
	self:Register({"noclipspeed", "nspeed", "ncspeed"}, function(pl, args)
		...
		--- REMOVED
		if (true) then 
			Core:Send(pl, "Print", {"General", "This command has been removed due to a bug, it will be added back soon!"})
			return
		end
		...
		if (not ty) or (not spd) then 
			Core:Send(pl, "Print", {"General", "Not enough arguments specified. (!noclipspeed <walk/shift/duck> <speed>)"})
			return
		end

		-- Invalid type
		if not table.HasValue({"walk", "shift", "duck"}, string.lower(ty)) then 
			Core:Send(pl, "Print", {"General", "Incorrect type. (Types: walk/shift/duck)"})
			return
		end
		...
		if (not speed) or (speed < 0) or (speed > 3500) then 
			Core:Send(pl, "Print", {"General", "Incorrect speed, enter a value from 0-3500."})
			return
		end
		...
		-- Inform 
		Core:Send(pl, "Print", {"General", ("Noclip type " .. ty .. " speed changed to " .. speed)})
	end)
	...
	self:Register( { "tp", "tpto", "goto", "teleport", "tele" }, function( ply, args )
		if not ply:GetNWInt("inPractice", false) then
			return Core:Send( ply, "Print", { "General", "You have to disable your timer first, either use noclip or enable checkpoints to allow for teleport." } )
		end
	...
	self:Register({"revote", "openrtv"}, function(ply, args)
		if (not RTV.VotePossible) then 
			Core:Send(ply, "Print", {"General", "There is no vote currently active."})
		else 
			local RTVSend = {}
	...
	self:Register( { "glock", "usp", "knife", "p90", "mp5", "crowbar", "deagle", "fiveseven", "m4a1", "ump45" }, function( ply, args )
		if ply.Spectating or ply:Team() == TEAM_SPECTATOR then
			...
			local maps_disallowed = {"bhop_sandtrap", "bhop_sandtrap2", "bhop_classicrainbowaux_fix", "kz_bhop_yonkoma"}

			if table.HasValue(maps_disallowed, game.GetMap()) then
				return Core:Send(ply, "Print", {"General", "Sorry, weapons are disabled on this map."})
			end
	...
	self:Register( { "bot" }, function( ply, args )
		Core:Send( ply, "Print", { "General", "This command has been renamed, please do !runs" } )
	end )
	...
	self:Register( { "setbot", "setrun", "runs", "botset" }, function( ply, args )
		if not args[ 1 ] then
			...
			if #list > 0 then
				return Core:Send( ply, "Print", { "General", "Runs on these styles are recorded and playable: " .. string.Implode( ", ", list ) .. " (Use !setbot Style to start playback.)" } )
			else
