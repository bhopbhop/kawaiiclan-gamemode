...
function Bot:LoadData()
	...
	if Core:Assert( Result, "nTime" ) then
		for _,Info in pairs( Result ) do
			...
			--Temp fix i guess till daddy fixes
			
			local Start = SysTime()
			local botnum = 1
			for s in string.gmatch(RunData, "([^%[([^%s]+]+)") do
				BotData[style][botnum] = {}
				local s = string.Replace(s,"]","")
				for k in string.gmatch(s,"[^%s, ]+") do
					table.insert(BotData[ style ][botnum], tonumber(k))
				end
				botnum = botnum + 1
			end
...
function Bot:EndRun( ply, nTime, nRank )
	...
	if BotInfo[ style ] and BotInfo[ style ].Time and nTime >= BotInfo[ style ].Time then
		return Core:Send( ply, "Print", { "Timer", Lang:Get( "BotSlow", { Timer:Convert( nTime - BotInfo[ style ].Time ) } ) } )
	end
