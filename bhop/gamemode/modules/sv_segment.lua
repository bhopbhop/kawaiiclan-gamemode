...
function Segment:SetWaypoint(client)
	...
	if (client.lastWaypoint > CurTime()) then 
		Core:Send(client, "Print", {"Timer", "You're setting too many waypoints too quickly! Please wait 2 seconds before setting another waypoint."})
		return
	end
...
function Segment:GotoWaypoint(client)
	...
	if (client.lastTele > CurTime()) then 
		Core:Send(client, "Print", {"Timer", "You're teleporting to waypoints too quickly! Please wait 0.5 seconds before teleporting again."})
		return
	end
