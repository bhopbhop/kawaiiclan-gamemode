...
-- Fonts
surface.CreateFont("hud.timer", {size = 17, weight = 800, font = "coolvetica"})
...
surface.CreateFont("hud.infotext", {font = "Tahoma", size = 13, weight = 100, antialias = true})
...
-- Colours
local SCORE_ONE = Color(31, 31, 34, 255)
local SCORE_TWO = Color(32, 32, 36, 255)
local SCORE_THREE = Color(34, 34, 38, 255)
local SCORE_ACCENT = Color(80, 30, 40, 255)
...
local SCORE_PLAYERS = "%s/%s Players Online"
local SCORE_CREDITS = "Gamemode based of Flow Network by Gravious, edited by justa"
...
local ranks = {"VIP", "VIP+", "Moderator", "Admin", "Super Admin", "Developer", "Owner"}
...
local function CreateScoreboard()
	...
	scoreboard = vgui.Create("EditablePanel")
		...
		scoreboard.Paint = function(self, width, height)
			...
			-- Title
			draw.SimpleText(SCORE_TITLE, "hud.title", 14, 12, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)

			-- Players online
			local text = string.format(SCORE_PLAYERS, #player.GetHumans(), game.MaxPlayers()-2)
			draw.SimpleText(text, "hud.title", width - 14, 12, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)

			-- Text
			draw.SimpleText(SCORE_CREDITS, "hud.credits", 14, height - 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
			draw.SimpleText("Click on a player's name for an options panel!", "hud.credits", width - 14, height - 20, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
		end
	...
	scoreboard.base = scoreboard:Add("DPanel")
		...
		scoreboard.base.Paint = function(self, width, height)
			...
			surface.SetDrawColor(color_black)
			surface.DrawOutlinedRect(0, 0, width, 22)

			-- Cool fucking way to calculate distances bro
			local distance = (width / 10)

			-- Rank
			draw.SimpleText("Rank", "hud.infotext", 12, 10, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText("Player", "hud.infotext", distance * 1.5, 10, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText("Style", "hud.infotext", distance * 4.5, 10, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText("Status", "hud.infotext", distance * 5.7, 10, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText("Personal Best", "hud.infotext", distance * 8.5, 10, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText("Ping", "hud.infotext", width - 12, 10, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
	...
	local function CreatePlayerRow(pl, isBot)
		...
		row.Paint = function(self, width, height)
			...
			draw.SimpleText(isBot and "WR Bot" or pRank[1], "hud.subtitle", 12, 20, isBot and color_white or pRank[2], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

			-- Player name
			local name = isBot and pl:GetNWString("BotName", "Loading...") or pl:Nick()
			if isBot and (name ~= "Loading..." and name ~= "Awaiting playback...") then
			...
			if (pl:SteamID() == "STEAM_0:0:53974417") then
				draw.SimpleText(name, "hud.subtitle", distance * 1.5, 20, Color( 0, 220 * abs(sin(CurTime())), 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			else
				draw.SimpleText(name, "hud.subtitle", distance * 1.5, 20, pl:GetObserverMode() ~= OBS_MODE_NONE and Color(150, 150, 150, 255) or color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			end

			-- Player style
			local style = Core:StyleName(pl:GetNWInt("Style", _C.Style.Normal))
			draw.SimpleText(style, "hud.subtitle", distance * 4.5, 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			...
			elseif (pl:GetObserverMode() ~= OBS_MODE_NONE) then 
				...
				if tgt and IsValid(tgt) and (tgt:IsPlayer() or tgt:IsBot()) then 
					local nm = (tgt:IsBot() and (tgt:GetNWString("BotName", "Loading...") .. "'s Bot") or tgt:Nick())
			...
			-- Calc 
			draw.SimpleText(status, "hud.subtitle", distance * 5.7, 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			...
			if not pl:IsBot() then
				...
				if (place == 0) then
					draw.SimpleText(pb, "hud.subtitle", distance * 8.5, 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
				else

					local w, h = surface.GetTextSize(place)

					draw.SimpleText(pb, "hud.subtitle", distance * 8.5 + 6 + w, 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
					draw.SimpleText("#" .. place, "hud.subinfo", distance * 8.5, 20, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
				end
			else
				draw.SimpleText(pb, "hud.subtitle", distance * 8.5, 20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
			end
			...
			draw.SimpleText(latency, "hud.subtitle", width - 12, 20, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
			...
			if (not scoreboard.moved) then
				...
				scoreboard:MoveTo(x - 150, y, 0.5, 0, -1, function()
					...
					scoreboard_playerrow.Paint = function(self, width, height)
						...
						surface.DrawRect(0, 0, width, height)
						surface.SetDrawColor(SCORE_TWO)
						surface.DrawRect(0, 177, width, 177)
						surface.SetDrawColor(color_black)
						surface.DrawOutlinedRect(0, 0, width, height)
						surface.SetFont("hud.title")

						-- Player name
						draw.SimpleText(scoreboard_playerrow.pl:IsBot() and scoreboard_playerrow.pl:GetNWString("BotName", "Loading...") or scoreboard_playerrow.pl:Nick(), "hud.title", 84, 26, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	

						-- Rank
						local rank = scoreboard_playerrow.pl:GetNWInt("AccessIcon", 0)
						rank = rank == 0 and "User" or ranks[rank]
						draw.SimpleText(scoreboard_playerrow.pl:IsBot() and "Bot" or rank, "hud.title", 84, 44, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	

						-- VIP
						draw.SimpleText("", "hud.title", 84, 56, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
						...
						if (not scoreboard_playerrow.pl:IsBot()) then
							...
							draw.SimpleText("Rank: " .. pRank[1], "hud.subtitle", 10, 104, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
							draw.SimpleText("Points: Feature disabled.", "hud.subtitle", 10, 122, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
							draw.SimpleText("Place: Feature disabled.", "hud.subtitle", 10, 140, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
							draw.SimpleText("Amount of WRs: Feature disabled.", "hud.subtitle", 10, 158, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)	
						end
					...
					scoreboard_playerrow.Combo:SetTextColor(Color(200, 200, 200))
					...
					local function NewButton(name, y, func)
						...
						b.Paint = function(self, width, height)
							...
							draw.SimpleText(name, "hud.subtitle", width / 2, height / 2, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)	
						end
